      ...
      <main className="min-h-full bg-transparent px-0 py-0">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="mt-3 text-4xl font-semibold text-zinc-900 dark:text-zinc-50">Admin Dashboard</h1>
              <p className="mt-3 max-w-2xl text-sm text-zinc-600 dark:text-zinc-400">Monitor clients, campaigns and activity for your team.</p>
            </div>
            <LogoutButton />
          </div>
            ...
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-medium uppercase tracking-widest text-zinc-500 dark:text-zinc-400">{stat.label}</p>
          ...
          <div className="mt-8">
            <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">Campaigns Overview</p>
                  <h2 className="mt-3 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">This Month</h2>
                </div>
                <button className="rounded-lg border border-zinc-200 bg-zinc-100 px-3 py-2 text-sm text-zinc-700 transition hover:bg-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700">This Month</button>
              </div>
          ...
          <div className="mt-8 grid gap-6 xl:grid-cols-[1.6fr_1fr]">
            <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <div className="mb-6 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Latest campaign updates across your account.</p>
                </div>
                <Link href="/admin/campaigns" className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-300 dark:hover:text-indigo-200">View All</Link>
              </div>
            ...
            <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <div className="mb-6 flex items-center justify-between gap-4">
                ...
                <Link href="/admin/clients" className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-300 dark:hover:text-indigo-200">View All</Link>
              </div>
              ...
              <div className="overflow-hidden rounded-3xl border border-zinc-200 dark:border-zinc-800">
                <table className="min-w-full divide-y divide-zinc-200 text-left text-sm dark:divide-zinc-800">
                  ...
                  <tbody className="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-950">
                    {clients.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-6 py-8 text-center text-zinc-500 dark:text-zinc-400">No clients found</td>
                      </tr>
                    ) : (
                      clients.map((client) => (
                        <tr key={String(client._id)} className="hover:bg-zinc-50 dark:hover:bg-zinc-900">
                          ...
                          <td className="px-6 py-4">
                            <Link href={`/admin/clients/${client._id}`} className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-300 dark:hover:text-indigo-200">View Campaigns</Link>
                          </td>
                        </tr>
                      ))
                    )}
