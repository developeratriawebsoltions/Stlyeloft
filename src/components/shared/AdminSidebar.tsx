...
const mainNav = [
  ...
  { label: "Add Client", href: "/admin/add-client", icon: User },
  ...
  return (
    <aside className="hidden h-screen w-64 shrink-0 flex-col border-r border-gray-200 bg-white lg:flex relative">
      ...
      <div className="flex flex-1 flex-col overflow-y-auto px-4 py-4">
        ...
        <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-widest text-emerald-500">
          Admin
        </p>
      ...
      <div className="border-t border-gray-200 px-4 py-4 space-y-2" ref={profileRef}>
        ...
        >
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-600 text-base font-semibold text-white">
            {user?.name
              ? user.name
                  .split(" ")
                  .map((part) => part[0])
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-sm text-gray-500">
              {user?.email ?? "admin@stlyeloft.com"}
            </p>
          </div>
        </button>
